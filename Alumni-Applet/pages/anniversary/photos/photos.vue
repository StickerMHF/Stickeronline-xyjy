    ...
    <view class="ann_news_content">
      ...
      <view class="photoList">
        <view
          ...
          :key="index"
          @click="hrefToPhotoListPage(item.user_id)"
        >
          <view class="imgBox">
      ...
      <view class="btnBox">
        ...
        <button class="textBtn uploadImg" @click="uploadPhoto">上传照片</button>
      </view>
    </view>
  </view>
</template>
...
<script>
import { getPhotoList, addPhoto, getUserList } from "@/api/cooperation.js";
export default {
  data() {
    return {
      title: "照片征集",
          ...
          files.map(file => {
            uni.uploadFile({
              ...
              success: uploadFileRes => {
                ...
                if (
                  JSON.stringify(file) ===
                  JSON.stringify(files[files.length - 1])
                ) {
                  that.addPhoto();
                }
                // macr36059    vh930040
