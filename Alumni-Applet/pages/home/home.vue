	...
	<view class="page_home">
		...
		<view class="ph-menu">
			<view class="bg-white">
				<view class="grid col-4 grid-square">
					<view @click="menuClickHandler(item.id)" class="bg-img" v-for="(item, index) in menus" :key="index" :style="[{ backgroundImage: 'url(' + menus[index] + ')' }]">
		...
		<view class="ph-menu">
			<scroll-view scroll-x class="bg-white nav" scroll-with-animation>
				<view class="cu-item cuItem" :class="index == tabCur ? 'text-green1' : ''" v-for="(item, index) in tabList" :key="index"
				 @tap="tabSelect" :data-id="item.id">
					{{ item.name }}
				</view>
			</scroll-view>

			<view class="phm-card cu-card case no-card" v-for="(item, index) in newsList" :key="index">
