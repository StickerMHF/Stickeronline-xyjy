...
export default {
  ...
  methods: {
    validateDictCode(rule, value, callback) {
      ...
      getAction("stickeronline/sys/duplicate/check",params).then(res => {
        if (res.success) {
          callback()
        } else {
    ...
    edit(record) {
      ...
      this.$nextTick(() => {
        ...
        this.form.setFieldsValue({
            'dictName':this.model.dictName, 
            'dictCode':this.model.dictCode, 
    ...
    handleOk() {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (!this.model.id) {
            formData.delFlag = '1'
            obj = postAction("stickeronline/sys/dict/add",formData)
          } else {
            obj = putAction("stickeronline/sys/dict/edit",formData)
          }
          obj
            .then(res => {
              if (res.success) {
