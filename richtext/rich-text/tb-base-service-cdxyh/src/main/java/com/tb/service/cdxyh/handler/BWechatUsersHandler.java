...
@RouteHandler("stickeronline/wechat/users")
@Api(tags = "新闻")
public class BWechatUsersHandler {
    private BWechatUsersAsyncService bWechatUsersAsyncService =
            AsyncServiceUtil.getAsyncServiceInstance(BWechatUsersAsyncService.class);

    @RouteMapping(value = "/add", method = RouteMethod.POST, order = 1)
    @ApiOperation(value = "微信用户")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "name", value = "微信用户", dataType = "String", paramType = "body", required = true),
            @ApiImplicitParam(name = "description", value = "描述", dataType = "String", paramType = "body"),
    })
    public Handler<RoutingContext> add() {
        return ctx -> {
            bWechatUsersAsyncService.add(CommonUtil.createCondition(ctx.request(), ctx.getBody()), res -> {
                if (res.succeeded()) {
    ...
    public Handler<RoutingContext> queryPageList() {
        return ctx -> {
            bWechatUsersAsyncService.queryPageList(CommonUtil.createCondition(ctx.request(), ctx.getBody()), res -> {
                if (res.succeeded()) {
                    HttpUtil.fireJsonResponse(ctx.response(), HTTP_OK,
                            ReplyObj.build().setSuccess(true).setResult(res.result()).setMsg("succeed"));
                } else {
    ...
    public Handler<RoutingContext> queryById() {
        return ctx -> {
            bWechatUsersAsyncService.queryById(CommonUtil.createCondition(ctx.request(), ctx.getBody()), res -> {
                if (res.succeeded()) {
                    HttpUtil.fireJsonResponse(ctx.response(), HTTP_OK,
                            ReplyObj.build().setSuccess(true).setResult(res.result()).setMsg("succeed"));
                } else {
    ...
    @ApiOperation(value = "编辑微信用户")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "微信用户id", dataType = "String", paramType = "body", required = true),
    ...
    public Handler<RoutingContext> edit() {
        return ctx -> {
            bWechatUsersAsyncService.edit(CommonUtil.createCondition(ctx.request(), ctx.getBody()), res -> {
                if (res.succeeded()) {
                    HttpUtil.fireJsonResponse(ctx.response(), HTTP_OK,
                            ReplyObj.build().setSuccess(true).setResult(res.result()).setMsg("succeed"));
                } else {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "微信用户id", dataType = "String", paramType = "query", required = true)
    })
    public Handler<RoutingContext> delete() {
        return ctx -> {
            bWechatUsersAsyncService.delete(CommonUtil.createCondition(ctx.request(), ctx.getBody()), res -> {
                if (res.succeeded()) {
                    HttpUtil.fireJsonResponse(ctx.response(), HTTP_OK,
                            ReplyObj.build().setSuccess(true).setResult(res.result()).setMsg("succeed"));
                } else {
